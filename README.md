...
# 📺 Bilibili - 免登入觀看 1080P

[![License: MIT](https://img.shields.io/badge/License-MIT-blue)](#-免責聲明-disclaimer)
[![Version](https://img.shields.io/badge/Version-1.0.0-green)](https://github.com/pthuang01/bilibili-unlogged-1080p)
[![Greasy Fork](https://img.shields.io/badge/Greasy%20Fork-安裝-960000)](https://greasyfork.org/)
...
本腳本全面解鎖限制，讓您<b>無需登入</b>即可直接暢享 <b>1080P 真·高清串流</b>！<br />
徹底攔截<b>登入彈窗</b>、<b>試看中斷降級</b>與<b>強制退出全螢幕</b>，享受最純粹乾淨的觀影體驗

[效果實測](#-效果實測) · [你需要嗎](#-為什麼需要這款腳本) · [主要特色](#-主要特色) · [安裝教學](#-安裝教學) · [功能解析](#-主要功能深度解析) · [常見問題](#-常見問題-faq) · [免責聲明](#-免責聲明-disclaimer)
...
<p align="center">
  ...
  <em>▲ <b>實測證明</b>：右上角明確顯示為<b>「未登入（登錄）」</b>狀態，播放器詳細統計資訊（Stats for Nerds）直接跑滿 <b>1920 × 1080 @ 30fps 真 1080P 高清！</b></em>
</p>
...
| **登入彈窗** | 🛑 頻繁彈出 `miniLogin` 遮擋畫面 | 🛡️ **底層攔截 + 即時銷毀，零彈窗** |
| **試看機制** | ⏱️ 幾十秒後強制中斷並降級畫質 | 🔓 **自動突破試看限制，持續高清播放** |
...
* 👉 **[從 Greasy Fork 安裝](https://greasyfork.org/)**
...
### 1. 🎯 真正 1080P DASH 串流注入
腳本從網路底層（Fetch / XMLHttpRequest）攔截播放器 API 請求，改寫清晰度參數並將當前畫質鎖定為 `QN: 80`。不是單純拉伸畫面的假高清，而是**真正向官方請求 1920×1080 解析度串流**。

### 2. 🚫 全方位登入彈窗粉碎器 (Anti-Popup Defense)
* 攔截 B 站內部 `miniLogin.show()` 與 `regist()` 呼叫。
* 透過 `MutationObserver` 在毫秒級內即時移除頁面上所有登入遮罩與彈窗容器。
* 全局注入 `!important` 隱藏樣式，雙重防線確保視線不被任何登入框遮擋。

### 3. 🛡️ 智慧播放守護（防暫停 & 防縮小）
* **全螢幕防踢出**：攔截非使用者主動觸發的 `exitFullscreen()` 行為，躺平看片不被干擾。
* **定時暫停攔截**：監聽使用者真實鍵盤（空白鍵、K、F 等）與滑鼠操作；由系統定時器觸發的惡意暫停一律直接攔截並自動恢復播放。
...
6. **遇到問題或有功能建議，該如何回報？**
   * 請前往 [GitHub Issues](https://github.com/pthuang01/bilibili-unlogged-1080p/issues) 或 [GreasyFork 反饋區](https://greasyfork.org/) 提出回報與建議，感謝~
